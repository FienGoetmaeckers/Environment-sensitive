# -*- coding: utf-8 -*-
"""
Created on Fri Dec  6 12:10:16 2024
...
from show_grid import show_bandit_masked
...
#willen we naar "zuiver" smooth en rough gedrag kijken?:
list_ls = [i for i in list_ls if i > np.median(list_lr)]
list_lr = [i for i in list_lr if i < np.median(list_ls)]
    
#we are aiming to create an 15x15 grid, with only two cells open
...
lr = 1
'''
so first, we try creating the visible grid
'''
...
hidden[tile_number] = False #save that this cell has been opened
observed_bandit[tile_number].append(reward)
show_bandit_masked(bandit, W, L, hidden)


'''
important check: is this environment equally likely rough as smooth?
'''
#estimate smoothness using the adjustable parameter kernel --> google this!
...
#visualize
bandit[best_cell] = 0
hidden[best_cell] = False
show_bandit_masked(bandit, W, L, hidden)


"""
show distributies voor gekozen cell
"""
smooth_exp = []
smooth_unc = []
smooth_max = []
